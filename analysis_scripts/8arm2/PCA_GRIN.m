...
d_m = [mean(d{1}(:,framesUsedForPCA,:),3)...
    mean(d{2}(:,framesUsedForPCA,:),3)];,...
    %mean(d{3}(:,framesUsedForPCA,:),3)];
%...
...
x = size(d_m,2)/numel(d)
C2 = mat2cell(COEFF,[x x ] , size(COEFF,2))
...
if calculatePCDistance
    ...
    x = size(d_proj,1)/numel(d)
    C2 = mat2cell(d_proj,[x x] , size(d_proj,2));
