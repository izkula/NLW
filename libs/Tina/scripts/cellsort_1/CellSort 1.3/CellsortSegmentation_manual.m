function [ica_segments, segmentlabel, cellmask, segcentroid] = CellsortSegmentation_manual(ica_filters, smwidth, thresh, arealims, plotting)
...
%     end
    ...
    if plotting && ~isempty(Lbig)
        ...
        if counter>1
            for a=1:counter-1
                contour(cellmask(:,:,a),0.5,'color','g');
...
%         end
        ...
        % added option to manually draw cells
        subplot(1,2,1)
        repeat=input('Any Cells? Enter 1 for yes, 0 for no');
        while repeat==1
            cellmask(:,:,counter)=roipoly;
            center=regionprops(cellmask(:,:,counter),'centroid');
            segcentroid(counter,:)=center.Centroid;
            hold on
            contour(cellmask(:,:,counter),0.5,'color','g');
            text(segcentroid(counter,1), segcentroid(counter,2), num2str(counter), 'horizontalalignment','c', 'verticalalignment','m','color','r')
            counter=counter+1;
            repeat=input('Another Cell? Enter 1 for yes, 0 for no');
        end
    end
