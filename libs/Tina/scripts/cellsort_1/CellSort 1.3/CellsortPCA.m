function [mixedsig, mixedfilters, CovEvals, covtrace, movm, ...
    movtm] = CellsortPCA(fn, flims, nPCs, dsamp, outputdir, badframes, doSubtractMean)
...
if ~exist('doSubtractMean', 'var') || isempty(doSubtractMean)
    doSubtractMean = false;
end
...
if nt < npix
    [covmat, mov, movm, movtm] = create_tcov(fn, pixw, pixh, useframes, nt, dsamp, doSubtractMean);
else
    [covmat, mov, movm, movtm] = create_xcov(fn, pixw, pixh, useframes, nt, dsamp, doSubtractMean);
end
...
toc
    ...
    function [covmat, mov, movm, movtm] = create_xcov(fn, pixw, pixh, useframes, nt, dsamp, doSubtractMean)
        %-----------------------
    ...
    function [covmat, mov, movm, movtm] = create_tcov(fn, pixw, pixh, useframes, nt, dsamp, doSubtractMean)
        %-----------------------
