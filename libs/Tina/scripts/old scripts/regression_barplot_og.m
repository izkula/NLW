...
for a=1:length(behav_cells)
   ...
   currthresh=mad(dfof(behav_cells(a),:))*6;
   active_trials=[];
   for b=1:numtrials3
       currtrial=currdfof(b,t_0:t_5);
       search=find(currtrial>currthresh);
       if ~isempty(search)
           active_trials=[active_trials b];
       end
   end
   peaks_pressed(a)=length(active_trials)/numtrials3;
end
...
for a=1:length(behav_cells)
   ...
   peaks_missed(a)=length(active_trials)/numtrials4;
end
