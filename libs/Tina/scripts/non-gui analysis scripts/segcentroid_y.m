...
for z=1:length(dates)
    ...
    centroid_shock=segcentroid(reward_only,2);
...
nshuff=1000;
m_centroid_nonmod=zeros(1,nshuff);
% nonmody_all=[];
for p=1:nshuff
    centroid_nonmod_all=[];
    for z=1:length(dates)
...
pval=length(find(m_centroid_nonmod<m_centroid_shock))/nshuff
pval=length(find(m_centroid_nonmod>m_centroid_shock))/nshuff
