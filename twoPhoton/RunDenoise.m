function RunDenoiseAndRegisterImageJ(vidpath, outdir, templatepath, imagejPath, imagejMacroPath)
...
doSilent = false;
if doSilent
    cmd = ['xvfb-run -a ', imagejPath, ' -macro ', imagejMacroPath, '/denoise.ijm ''',vidpath,' ', outdir, ' ', templatepath '''']
else
    cmd = [imagejPath, ' -macro ', imagejMacroPath, '/denoise.ijm ''',vidpath,' ', outdir, ' ', templatepath '''']
end
disp(outdir);
