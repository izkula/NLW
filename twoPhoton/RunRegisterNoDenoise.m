...
doSilent = false;
if doSilent
    cmd = ['xvfb-run -a ', imagejPath, ' -macro ', imagejMacroPath, '/turboregFromStack.ijm ''',vidpath,' ', outdir, ' ', templatepath '''']
else
   cmd = [imagejPath, ' -macro ', imagejMacroPath, '/turboregFromStack.ijm ''',vidpath,' ', outdir, ' ', templatepath ''''] 
end
disp(outdir);
